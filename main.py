...
def change_antenna(driver, antenna_type):
    driver.find_element('id','antenna_mode_select').click()
    driver.find_element('id',f'antenna_mode_select_list_item_{antenna_type}').click()
...
with Connection(f'http://admin:{PSW}@{IP}/') as connection:
    ...
    if enbid != ENB_ID or ant_type != 2:
        ...
        if enbid != ENB_ID:
            ...
            change_antenna(driver, 1)
            count = 0
            while (int(hex(int(client.device.signal()['cell_id']))[0:-2], 16) != ENB_ID and count < 15):
                time.sleep(1)
                count += 1
